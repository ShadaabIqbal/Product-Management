...
const getCart = async function (req, res) {
    try {
        ...
        if (!cartPresent) return res.status(400).send({ status: false, message: "cart not found for this user" })
        return res.status(200).send({ status: true, message: "Success", data: cartPresent })
    } catch (error) {
        return res.status(500).send({ status: false, message: error.message })
    }
...
const deleteCart = async function (req, res) {
    try {
        ...
        let price = 0
        let noOfItems = 0
        let savedData = await cartModel.findOneAndUpdate({ userId: userId }, { $set: { items: items, totalPrice: price, totalItems: noOfItems } }, { new: true })
        return res.status(204).send({ status: true, message: "Success", data: savedData })
    } catch (error) {
        return res.status(500).send({ status: false, message: error.message })
    }
}
